...
def get_u(lidx: int, v_prev: Union[int, float, None]=None, u_prev: Union[int, float, None]=None) -> Union[int, float]:
    """
    计算 u 值，根据公式
    u1 = d1
    u2 = d2 - (a2 * v1 / u1)
    ...
    if lidx == 0:
        ...
        if v_prev is None or u_prev is None:
            raise ValueError("v_prev and u_prev must be provided for non-first row.")   # 不是第一行的时候必须提供 v_prev 和 u_prev
...
def get_y(lidx: int, v_prev: Union[int, float]=None, u_prev: Union[int, float]=None) -> Union[int, float]:
    """
    计算 y 值，根据公式
    y1 = b1
    yi = bi - (ai * yi-1) / ui-1
...
def get_c(lidx: int) -> Union[int, float]:
    ...
    if lidx == len(MATRIX) - 1:
        raise ValueError("lidx must be less than the last index to get c value.")   # c[-1] 是不存在的
    return MATRIX[lidx][lidx + 1]   # ci 的值

# 回代
def backward(lidx: int, y_current: Union[int, float], u_current: Union[int, float], x_next: Union[int, float]):
    """
    回代求解过程，根据公式
